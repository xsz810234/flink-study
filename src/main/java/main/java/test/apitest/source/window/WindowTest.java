...
//            );
        DataStreamSource<Event> dataStreamSource = env.fromElements(new Event("mary", "./abc", 1000l)
            ,new Event("mary", "./abc", 2000l)
        );
        SingleOutputStreamOperator<Event> eventSingleOutputStreamOperator = dataStreamSource.assignTimestampsAndWatermarks(WatermarkStrategy.<Event>forBoundedOutOfOrderness(Duration.ofSeconds(1))
        ...
        SingleOutputStreamOperator<Tuple2<String, Long>> clickStream = eventSingleOutputStreamOperator.map(new MapFunction<Event, Tuple2<String, Long>>() {
        ...
        clickStream.assignTimestampsAndWatermarks( WatermarkStrategy.<Tuple2<String, Long>>forBoundedOutOfOrderness(Duration.ofSeconds(1))
        ...
        clickStream.keyBy(data -> data.f0)
